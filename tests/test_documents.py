...
class TestDocumentOperations:
    ...
    async def test_create_document(self, authenticated_client: CFMSTestClient):
        ...
        assert isinstance(response["data"]["document_id"], str), (
            "document_id should be a string"
        )
        assert len(response["data"]["document_id"]) > 0, (
    ...
    async def test_get_document(
        ...
        try:
            response = await authenticated_client.get_document(
    ...
    async def test_delete_document(self, authenticated_client: CFMSTestClient):
        ...
        try:
            create_response = await authenticated_client.create_document(
                "Document to Delete"
            )
        except Exception as e:
